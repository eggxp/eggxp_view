...
class ByteBufferException
{
    public:
        ...
        {
            GetLog()->Error("ERROR: Attempted to [%s] in ByteBuffer (pos: %d size: %d) value with size: %d",
...
class ByteBuffer
{
    public:
        ...
        {
			GetLog()->Warn("STORAGE_SIZE: %llu", (unsigned long)size() );
            for(uint32 i = 0; i < size(); ++i)
                GetLog()->Warn("%u - ", read<uint8>(i) );
            GetLog()->Warn(" ");
        }
        ...
        void hexlike() const
        {
            ...
            for(uint32 i = 0; i < size(); ++i)
            {
                if ((i == (j * 8)) && ((i != (k * 16))))
                {
                    ...
                    {
                        GetLog()->Warn("| 0%X ", read<uint8>(i) );
                    }
                    else
                    {
                        GetLog()->Warn("| %X ", read<uint8>(i) );
                    }
                    ++j;
                }
                else if (i == (k * 16))

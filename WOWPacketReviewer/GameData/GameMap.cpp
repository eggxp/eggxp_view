...
bool GridMap::loadData(char *filename)
{
        ...
        header.versionMagic == *((uint32 const*)(MAP_VERSION_MAGIC)))
    {
        ...
        {
            GetLog()->Error("Error loading map area data\n");
            fclose(in);
            return false;
        }
        // loadup height data
        ...
        {
            GetLog()->Error("Error loading map height data\n");
            fclose(in);
            return false;
        }
        // loadup liquid data
        ...
        {
            GetLog()->Error("Error loading map liquids data\n");
            fclose(in);
            return false;
        }
        fclose(in);
    ...
    GetLog()->Error("Map file '%s' is non-compatible version (outdated?). Please, create new using ad.exe program.", filename);
    fclose(in);
    return false;
}
...
GameMap::~GameMap()
{
    for(DWORD x=0; x<m_MapGrids.size(); x++)
    {
        for(DWORD y=0; y<m_MapGrids[x].size(); y++)
        {
            if(m_MapGrids[x][y])
            {
                delete  m_MapGrids[x][y];
            }
        }
    }
}

void        GameMap::Init(int mapid, String path)
{
    m_MapID = mapid;
    m_Path = path;

    m_MapEntry = GetMapStore()->LookupEntry(m_MapID);

    for(int x=0; x<MAX_NUMBER_OF_GRIDS; x++)
    {
        MapGridListType curtype;
        m_MapGrids.push_back(curtype);
        for(int y=0; y<MAX_NUMBER_OF_GRIDS; y++)
        {
            m_MapGrids[x].push_back(NULL);
        }
    }
}

void        GameMap::LoadMap(int gx, int gy)
{
    if(m_MapGrids[gx][gy])
        return;
        
    String fileName = FormatStr("%03d%02d%02d", m_MapID, gx, gy);
    String fullName = m_Path + "maps\\" + fileName + ".map";
    if(!FileExists(fullName))
        return;

	m_MapGrids[gx][gy] = new GridMap();
	AnsiString fullNameAnsi = fullName;
    m_MapGrids[gx][gy]->loadData(fullNameAnsi.c_str());
}

void        GameMap::LoadVMap(int gx, int gy)
{
}

void        GameMap::LoadMapAndVMap(int gx, int gy)
{
    LoadMap(gx, gy);
    if(!m_MapEntry->Instanceable())
    {
        LoadVMap(gx, gy);
    }
}

GridMap *   GameMap::GetGridMap(int gx, int gy)
{
    if(gx >= (int)m_MapGrids.size())
        return NULL;
    if(gy >= (int)m_MapGrids[gx].size())
        return NULL;
    if(m_MapGrids[gx][gy])
        return  m_MapGrids[gx][gy];

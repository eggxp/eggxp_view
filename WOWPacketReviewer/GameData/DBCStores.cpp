...
//        if(build == (DWORD)accepted_versions[i])
//            return true;
//
//    return false;
//}
...
static bool LoadDBC_assert_print(uint32 fsize,uint32 rsize, const std::string& filename)
{
    ...
    GetLog()->Error("ERROR: Size of '%s' setted by format string (%u) not equal size of C++ structure (%u).",
                    file.c_str(),
                    fsize,
                    rsize);
...
//    {
//        if (build)
//            GetLog()->Error("Found DBC files for build %u but mangosd expected DBC for one from builds: %s Please extract correct DBC files.", build, AcceptableClientBuildsListStr().c_str());
//        else
//            GetLog()->Error("Incorrect DataDir value in mangosd.conf or not found build info (outdated DBC files). Required one from builds: %s Please extract correct DBC files.",AcceptableClientBuildsListStr().c_str());
//        return false;
//    }
...
//        #endif
	...
	{
		String path = dataPath.c_str();
        GetLog()->Error("\nIncorrect DataDir value in mangosd.conf or ALL required *.dbc files (%d) not found by path: %sdbc",DBCFilesCount,path.c_str());
        return false;
    }
    else if(!bad_dbc_files.empty() )
    {
		...
		String bcbStr = str.c_str();
        GetLog()->Error("\nSome required *.dbc files (%u from %d) not found or not compatible:\n%s",(uint32)bad_dbc_files.size(),DBCFilesCount,bcbStr.c_str());
        return false;
    }
	...
	{
		String build = AcceptableClientBuildsListStr().c_str();
        GetLog()->Error("\nYou have mixed version DBC files. Please re-extract DBC files for one from client build: %s",build.c_str());
        return false;
    }

    GetLog()->Warn( ">> Initialized %d data stores", DBCFilesCount );
    return true;
}
...
MANGOS_DLL_SPEC DBCStorage<TaxiPathEntry>  const* GetTaxiPathStore(){return &sTaxiPathStore;}
MANGOS_DLL_SPEC DBCStorage<SkillLineAbilityEntry>  const* GetSkillLineAbilityStore(){return &sSkillLineAbilityStore;}
MANGOS_DLL_SPEC DBCStorage<ItemRandomSuffixEntry>  const* GetItemRandomSuffixStore(){return &sItemRandomSuffixStore;}
MANGOS_DLL_SPEC DBCStorage<QuestXPLevel>  const* GetQuestXPLevelStore(){return &sQuestXPLevelStore;}
MANGOS_DLL_SPEC DBCStorage<TotemCategoryEntry>  const* GetTotemCategoryStore(){return &sTotemCategoryStore;}
MANGOS_DLL_SPEC DBCStorage<GlyphSlotEntry>  const* GetGlyphSlotStore(){return &sGlyphSlotStore;}
MANGOS_DLL_SPEC DBCStorage<QuestSortEntry>  const* GetQuestSortStore(){return &sQuestSortStore;}
MANGOS_DLL_SPEC DBCStorage<MovieEntry>  const* GetMovieStore(){return &sMovieStore;}
MANGOS_DLL_SPEC DBCStorage<RandomPropertiesPointsEntry>  const* GetRandomPropertiesPointsStore(){return &sRandomPropertiesPointsStore;}
MANGOS_DLL_SPEC DBCStorage<ScalingStatDistributionEntry>  const* GetScalingStatDistributionStore(){return &sScalingStatDistributionStore;}
MANGOS_DLL_SPEC DBCStorage<ScalingStatValuesEntry>  const* GetScalingStatValuesStore(){return &sScalingStatValuesStore;}
MANGOS_DLL_SPEC DBCStorage<WorldSafeLocsEntry>  const* GetWorldSafeLocsStore(){return &sWorldSafeLocsStore;}
MANGOS_DLL_SPEC DBCStorage<WorldMapOverlayEntry>  const* GetWorldMapOverlayStore(){return &sWorldMapOverlayStore;}
MANGOS_DLL_SPEC DBCStorage<DurabilityCostsEntry>  const* GetDurabilityCostsStore(){return &sDurabilityCostsStore;}
MANGOS_DLL_SPEC DBCStorage<ItemRandomPropertiesEntry>  const* GetItemRandomPropertiesStore(){return &sItemRandomPropertiesStore;}
MANGOS_DLL_SPEC DBCStorage<GtOCTRegenHPEntry>  const* GetGtOCTRegenHPStore(){return &sGtOCTRegenHPStore;}
MANGOS_DLL_SPEC DBCStorage<TalentTabEntry>  const* GetTalentTabStore(){return &sTalentTabStore;}
MANGOS_DLL_SPEC DBCStorage<SpellItemEnchantmentEntry>  const* GetSpellItemEnchantmentStore(){return &sSpellItemEnchantmentStore;}
MANGOS_DLL_SPEC DBCStorage<VehicleEntry>  const* GetVehicleStore(){return &sVehicleStore;}
MANGOS_DLL_SPEC DBCStorage<ChrClassesEntry>  const* GetChrClassesStore(){return &sChrClassesStore;}
MANGOS_DLL_SPEC DBCStorage<BarberShopStyleEntry>  const* GetBarberShopStyleStore(){return &sBarberShopStyleStore;}
MANGOS_DLL_SPEC DBCStorage<MailTemplateEntry>  const* GetMailTemplateStore(){return &sMailTemplateStore;}
MANGOS_DLL_SPEC DBCStorage<SpellEntry>  const* GetSpellStore(){return &sSpellStore;}
MANGOS_DLL_SPEC DBCStorage<AreaTriggerEntry>  const* GetAreaTriggerStore(){return &sAreaTriggerStore;}
MANGOS_DLL_SPEC DBCStorage<MapEntry>  const* GetMapStore(){return &sMapStore;}
MANGOS_DLL_SPEC DBCStorage<ChrRacesEntry>  const* GetChrRacesStore(){return &sChrRacesStore;}
MANGOS_DLL_SPEC DBCStorage<FactionEntry>  const* GetFactionStore(){return &sFactionStore;}
MANGOS_DLL_SPEC DBCStorage<ItemLimitCategoryEntry>  const* GetItemLimitCategoryStore(){return &sItemLimitCategoryStore;}
MANGOS_DLL_SPEC DBCStorage<AuctionHouseEntry>  const* GetAuctionHouseStore(){return &sAuctionHouseStore;}
MANGOS_DLL_SPEC DBCStorage<SpellRadiusEntry>  const* GetSpellRadiusStore(){return &sSpellRadiusStore;}
MANGOS_DLL_SPEC DBCStorage<HolidaysEntry>  const* GetHolidaysStore(){return &sHolidaysStore;}
MANGOS_DLL_SPEC DBCStorage<ItemBagFamilyEntry>  const* GetItemBagFamilyStore(){return &sItemBagFamilyStore;}
MANGOS_DLL_SPEC DBCStorage<CreatureSpellDataEntry>  const* GetCreatureSpellDataStore(){return &sCreatureSpellDataStore;}
...
MANGOS_DLL_SPEC DBCStorage<AchievementCriteriaEntry>  const* GetAchievementCriteriaStore(){return &sAchievementCriteriaStore;}
MANGOS_DLL_SPEC DBCStorage<GtCombatRatingsEntry>  const* GetGtCombatRatingsStore(){return &sGtCombatRatingsStore;}
MANGOS_DLL_SPEC DBCStorage<GtChanceToSpellCritBaseEntry>  const* GetGtChanceToSpellCritBaseStore(){return &sGtChanceToSpellCritBaseStore;}
MANGOS_DLL_SPEC DBCStorage<GtChanceToMeleeCritBaseEntry>  const* GetGtChanceToMeleeCritBaseStore(){return &sGtChanceToMeleeCritBaseStore;}
MANGOS_DLL_SPEC DBCStorage<EmotesTextEntry>  const* GetEmotesTextStore(){return &sEmotesTextStore;}
MANGOS_DLL_SPEC DBCStorage<CreatureFamilyEntry>  const* GetCreatureFamilyStore(){return &sCreatureFamilyStore;}
MANGOS_DLL_SPEC DBCStorage<AchievementEntry>  const* GetAchievementStore(){return &sAchievementStore;}
MANGOS_DLL_SPEC DBCStorage<SpellFocusObjectEntry>  const* GetSpellFocusObjectStore(){return &sSpellFocusObjectStore;}
MANGOS_DLL_SPEC DBCStorage<SpellShapeshiftEntry>  const* GetSpellShapeshiftStore(){return &sSpellShapeshiftStore;}
MANGOS_DLL_SPEC DBCStorage<SoundEntriesEntry>  const* GetSoundEntriesStore(){return &sSoundEntriesStore;}
MANGOS_DLL_SPEC DBCStorage<TalentEntry>  const* GetTalentStore(){return &sTalentStore;}
MANGOS_DLL_SPEC DBCStorage<EmotesEntry>  const* GetEmotesStore(){return &sEmotesStore;}
MANGOS_DLL_SPEC DBCStorage<CurrencyTypesEntry>  const* GetCurrencyTypesStore(){return &sCurrencyTypesStore;}
MANGOS_DLL_SPEC DBCStorage<ItemSetEntry>  const* GetItemSetStore(){return &sItemSetStore;}
MANGOS_DLL_SPEC DBCStorage<ItemEntry>  const* GetItemStore(){return &sItemStore;}
MANGOS_DLL_SPEC DBCStorage<ItemExtendedCostEntry>  const* GetItemExtendedCostStore(){return &sItemExtendedCostStore;}
MANGOS_DLL_SPEC DBCStorage<CharStartOutfitEntry>  const* GetCharStartOutfitStore(){return &sCharStartOutfitStore;}
MANGOS_DLL_SPEC DBCStorage<SpellRangeEntry>  const* GetSpellRangeStore(){return &sSpellRangeStore;}
MANGOS_DLL_SPEC DBCStorage<QuestFactionRewardEntry>  const* GetQuestFactionRewardStore(){return &sQuestFactionRewardStore;}
MANGOS_DLL_SPEC DBCStorage<SpellRuneCostEntry>  const* GetSpellRuneCostStore(){return &sSpellRuneCostStore;}
MANGOS_DLL_SPEC DBCStorage<SpellItemEnchantmentConditionEntry>  const* GetSpellItemEnchantmentConditionStore(){return &sSpellItemEnchantmentConditionStore;}
MANGOS_DLL_SPEC DBCStorage<LockEntry>  const* GetLockStore(){return &sLockStore;}
MANGOS_DLL_SPEC DBCStorage<DurabilityQualityEntry>  const* GetDurabilityQualityStore(){return &sDurabilityQualityStore;}
MANGOS_DLL_SPEC DBCStorage<BattlemasterListEntry>  const* GetBattlemasterListStore(){return &sBattlemasterListStore;}
MANGOS_DLL_SPEC DBCStorage<SpellDurationEntry>  const* GetSpellDurationStore(){return &sSpellDurationStore;}
MANGOS_DLL_SPEC DBCStorage<CreatureTypeEntry>  const* GetCreatureTypeStore(){return &sCreatureTypeStore;}
MANGOS_DLL_SPEC DBCStorage<GtRegenMPPerSptEntry>  const* GetGtRegenMPPerSptStore(){return &sGtRegenMPPerSptStore;}
MANGOS_DLL_SPEC DBCStorage<GlyphPropertiesEntry>  const* GetGlyphPropertiesStore(){return &sGlyphPropertiesStore;}
MANGOS_DLL_SPEC DBCStorage<GameObjectDisplayInfoEntry>  const* GetGameObjectDisplayInfoStore(){return &sGameObjectDisplayInfoStore;}
MANGOS_DLL_SPEC DBCStorage<GtRegenHPPerSptEntry>  const* GetGtRegenHPPerSptStore(){return &sGtRegenHPPerSptStore;}
MANGOS_DLL_SPEC DBCStorage<CinematicSequencesEntry>  const* GetCinematicSequencesStore(){return &sCinematicSequencesStore;}
MANGOS_DLL_SPEC DBCStorage<GtChanceToSpellCritEntry>  const* GetGtChanceToSpellCritStore(){return &sGtChanceToSpellCritStore;}
MANGOS_DLL_SPEC DBCStorage<SkillLineEntry>  const* GetSkillLineStore(){return &sSkillLineStore;}
MANGOS_DLL_SPEC DBCStorage<GtChanceToMeleeCritEntry>  const* GetGtChanceToMeleeCritStore(){return &sGtChanceToMeleeCritStore;}
MANGOS_DLL_SPEC DBCStorage<VehicleSeatEntry>  const* GetVehicleSeatStore(){return &sVehicleSeatStore;}
MANGOS_DLL_SPEC DBCStorage<FactionTemplateEntry>  const* GetFactionTemplateStore(){return &sFactionTemplateStore;}
MANGOS_DLL_SPEC DBCStorage<TaxiNodesEntry>  const* GetTaxiNodesStore(){return &sTaxiNodesStore;}
MANGOS_DLL_SPEC DBCStorage<BankBagSlotPricesEntry>  const* GetBankBagSlotPricesStore(){return &sBankBagSlotPricesStore;}
MANGOS_DLL_SPEC DBCStorage<SummonPropertiesEntry>  const* GetSummonPropertiesStore(){return &sSummonPropertiesStore;}
MANGOS_DLL_SPEC DBCStorage<GemPropertiesEntry>  const* GetGemPropertiesStore(){return &sGemPropertiesStore;}
MANGOS_DLL_SPEC DBCStorage<AreaGroupEntry>  const* GetAreaGroupStore(){return &sAreaGroupStore;}
MANGOS_DLL_SPEC DBCStorage<StableSlotPricesEntry>  const* GetStableSlotPricesStore(){return &sStableSlotPricesStore;}
MANGOS_DLL_SPEC DBCStorage<SpellCastTimesEntry>  const* GetSpellCastTimesStore(){return &sSpellCastTimesStore;}
MANGOS_DLL_SPEC DBCStorage<SpellDifficultyEntry>  const* GetSpellDifficultyStore(){return &sSpellDifficultyStore;}
MANGOS_DLL_SPEC DBCStorage<GtBarberShopCostBaseEntry>  const* GetGtBarberShopCostBaseStore(){return &sGtBarberShopCostBaseStore;}
//MANGOS_DLL_SPEC DBCStorage<AchievementCategoryEntry>  const* GetAchievementCategoryStore(){return &sAchievementCategoryStore;}
MANGOS_DLL_SPEC DBCStorage<MapDifficultyEntry>  const* GetMapDifficultyStore(){return &sMapDifficultyStore;}
//MANGOS_DLL_SPEC DBCStorage<ItemDisplayInfoEntry>  const* GetItemDisplayInfoStore(){return &sItemDisplayInfoStore;}
MANGOS_DLL_SPEC DBCStorage<WorldMapAreaEntry>  const* GetWorldMapAreaStore(){return &sWorldMapAreaStore;}
MANGOS_DLL_SPEC DBCStorage<AreaTableEntry>  const* GetAreaTableStore(){return &sAreaStore;}
MANGOS_DLL_SPEC DBCStorage<PvPDifficultyEntry>  const* GetPvPDifficultyStore(){return &sPvPDifficultyStore;}
//MANGOS_DLL_SPEC DBCStorage<GtOCTRegenMPEntry>  const* GetGtOCTRegenMPStore(){return &sGtOCTRegenMPStore;}
MANGOS_DLL_SPEC DBCStorage<ChatChannelsEntry>  const* GetChatChannelsStore(){return &sChatChannelsStore;}
MANGOS_DLL_SPEC DBCStorage<TaxiPathNodeEntry>  const* GetTaxiPathNodeStore(){return &sTaxiPathNodeStore;}

...
namespace VMAP
{
	...
	//=========================================================
	void VMapManager::DumpTreeNodes(vector<const VMAP::SubModel *> *nodes)
	{
		Array<unsigned int > keyArray = iInstanceMapTrees.getKeys();
		for(int i=0;i<keyArray.size(); ++i)
		{
			iInstanceMapTrees.get(keyArray[i])->DumpTreeNodes(nodes);
		}
	}
    ...
    */
    ...
    //=========================================================
	void MapTree::DumpTreeNodes(vector<const VMAP::SubModel *> *nodes)
	{
		G3D::Table<std::string, ManagedModelContainer *>::Iterator it = 
			iLoadedModelContainer.begin();
		for(; it != iLoadedModelContainer.end(); it++)
		{
			for(uint32 i=0; i<it->value->getNSubModel(); i++)
			{
				const SubModel& submodel = it->value->getSubModel(i);
				nodes->push_back(&submodel);
			}

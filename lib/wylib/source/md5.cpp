...
{
  //initialise data with current checksum
  ULONG a = m_lMD5[0];
  ULONG b = m_lMD5[1];
  ULONG c = m_lMD5[2];
  ULONG d = m_lMD5[3];
...
//---------------------------------------------------------------------------
LPSTR CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
  char cBuff[3];
  cBuff[2] = 0;
  lpMD5[0] = 0;
